...
\end{split}
\end{equation} em que $i, j, k, \ell$ são números inteiros que denotam a quantidade de repetições da operação associada perante multiplicação. Os valores destes inteiros foram: $i = 2$, $j = \{0,1\}$ (realização ou não das operações de Pooling e Dropout), $k = \{1,2\}$ e $\ell = \{ 1,2,3 \}$.\todo[inline]{Revisar estas repetições.}. Considerando esta abordagem de adaptação, foram então propostas $9$ CNNs diferentes a serem treinadas e testadas, conforme a abordagem de validação previamente descrita.
